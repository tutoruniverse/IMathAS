<template>
  <div class="questionwrap questionpane" ref="main">
    ...
    />
    <div
      v-else-if="showScore && questionData.canregen"
      class="scoreresult neutral"
      tabindex="-1"
    >
      <button type="button" @click="trySimilar">
    ...
    <div
      ...
      ref="thisqwrap"
    />
    <question-helps v-if="showHelps" :qn="qn" />

    <div v-if="showWork && questionContentLoaded">
      <button v-if="getwork !== 2" @click="showWorkInput = !showWorkInput">
...
export default {
  ...
  computed: {
    ...
    hasSeqNext() {
      return (
        this.questionData.jsparams && this.questionData.jsparams.hasseqnext
      );
    },
    hasSubmitAll() {
      return (
        this.questionData.jsparams && this.questionData.jsparams.submitall === 1
      );
    },
    buttonsOk() {
      return (
    ...
    showSubmit() {
      return (
        ...
        (store.assessInfo.displaymethod !== "livepoll" || this.state === 2)
      );
    },
    showNext() {
      return (
    ...
    submitClass() {
      return store.assessInfo.submitby === "by_assessment" ||
        !this.questionData.canretry_primary
        ? "secondary"
        : "primary";
    },
    showScore() {
    ...
    showHelps() {
      return (
        (store.assessInfo.hasOwnProperty("help_features") &&
          (store.assessInfo.help_features.message === true ||
            store.assessInfo.help_features.forum > 0)) ||
        (this.questionData.jsparams &&
          this.questionData.jsparams.helps.length > 0)
      );
    },
    errorsToShow() {
      let errors = [];
      if (
        store.assessInfo.hasOwnProperty("scoreerrors") &&
    ...
    showWork() {
      return (
        (this.getwork === 1 &&
          store.assessInfo.questions[this.qn].showwork & 1) ||
  ...
  methods: {
    ...
    addDirtyTrackers() {
      window
        .$("#questionwrap" + this.qn)
        .find("input[name],select[name],textarea[name]")
        .off("focus.dirtytrack")
        .off("change.dirtytrack")
        .off("input.dirtytrack")
        ...
        .on("change.dirtytrack", function () {
          ...
          if (this.type === "radio" || this.type === "checkbox") {
            ...
            val !== window.$(this).attr("data-lastval") &&
            val !== ""
          ) {
            changed = true;
          }
    ...
    disableOutOfTries() {
      ...
      for (const pn in this.questionData.parts) {
        ...
        if (this.questionData.parts[pn].try >= trymax) {
          ...
          if (
            Object.keys(this.questionData.parts).length === 1 &&
            Object.keys(this.questionData.jsparams).length > 1
          ) {
            // Only one "part" listed, but multiple input boxes.
            // Probably conditional. Disable all boxes
            regex = new RegExp(
              "^(qn|tc|qs)(" + this.qn + "\\b|" + (this.qn + 1) + "\\d{3}\\b)"
            );
          } else if (pn === 0) {
            regex = new RegExp(
              "^(qn|tc|qs)(" +
            ...
            regex = new RegExp(
              "^(qn|tc|qs)" + ((this.qn + 1) * 1000 + pn * 1) + "\\b"
            );
          }
          window
            .$("#questionwrap" + this.qn)
...
.mathquill-math-field.ansgrn,
input[type="text"].ansred,
.mathquill-math-field.ansred,
input[type="text"].ansyel,
.mathquill-math-field.ansyel,
input[type="text"].ansorg,
.mathquill-math-field.ansorg {
...
.mathquill-math-field.ansorg {
  ...
  background: right no-repeat
    url("data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCIgd2lkdGg9IjE2IiBoZWlnaHQ9IjE2IiBzdHJva2U9InJnYigyNTUsODUsMCkiIHN0cm9rZS13aWR0aD0iMyIgZmlsbD0ibm9uZSI+PHBhdGggZD0iTTE4IDYgTDYgMTggTTYgNiBMMTggMTgiIC8+PC9zdmc+");
}
</style>
