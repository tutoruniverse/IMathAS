...
function makeanswerbox($anstype, $qn, $la, $options,$multi,$colorbox='',$tipFormat='latex') {
	...
	if ($anstype == "number") {
		...
		if (in_array('list',$ansformats) || in_array('exactlist',$ansformats) ||  in_array('orderedlist',$ansformats)) {
			if (in_array('integer',$ansformats)) {
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as a list of integers separated with commas: Example: \\[-4, 3, 2\\]') . "\n\n";
				}
				else {
				...
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as a list of integer or decimal numbers separated with commas: Examples: \\[-4, 3, 2.5172\\]') . "\n\n";
				} else {
			...
			if (in_array('integer',$ansformats)) {
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as a set of integers separated with commas: Example: \\[{-4, 3, 2}\\]') . "\n\n";
				} else {
				...
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as a set of integer or decimal numbers separated with commas: Example: \\[{-4, 3, 2.5172}\\]') . "\n\n";
				} else {
			...
			if (in_array('integer',$ansformats)) {
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as an integer.  Examples: \\[3, -4, 0\\]') . "\n\n";
				} else {
				...
				if ($tipFormat == 'latex') {
					$tip = _('Enter your answer as an integer or decimal number.  Examples: \\[3, -4, 5.5172\\]') . "\n\n";
				} else {
		...
		if (isset($reqdecimals)) {
			if (substr((string)$reqdecimals,0,1)=='=') {
				...
				if ($tipFormat == 'latex') {
					$tip .= "\n\n" . sprintf(_('Your answer should include exactly %d decimal places.'), $reqdecimals);
				} else {
				...
				if ($tipFormat == 'latex') {
					$tip .= "\n\n" . sprintf(_('Your answer should be accurate to at least %d decimal places.'), $reqdecimals);
				} else {
		...
		if (isset($reqsigfigs)) {
			if ($reqsigfigs[0]=='=') {
				...
				if ($tipFormat == 'latex') {
					$tip .= "\n\n" . sprintf(_('Your answer should have between %d and %d significant figures.'), $reqsigfigparts[0], $reqsigfigparts[1]);
				} else {
		...
		if (isset($answersize)) {
			...
			if ($tipFormat == 'latex') {
				...
				$tip = $shorttip."\n\n".formathint(_('each element of the matrix'),$ansformats,isset($reqdecimals)?$reqdecimals:null,'calcmatrix');
			} else {
				$shorttip = _('Enter your answer as a matrix, like ((2,3,4),(1,4,5))');
		...
		if ($displayformat == 'point') {
			...
			if ($tipFormat == 'latex') {
				$tip = _('Enter your answer as a vector.  Example: \\[<2,5.5>\\]') . "\n\n";
			} else {
		...
		if (isset($reqdecimals)) {
			if ($tipFormat == 'latex') {
				$tip .= sprintf(_('Each value should be accurate to %d decimal places.'), $reqdecimals)."\n\n";
			} else {
...
function formathint($eword,$ansformats,$reqdecimals,$calledfrom, $islist=false,$doshort=false, $format="latex") {
	if ($format == "html") {
		return formathint_html($eword,$ansformats,$reqdecimals,$calledfrom,$islist,$doshort);
	} else {
		return formathint_latex($eword,$ansformats,$reqdecimals,$calledfrom,$islist,$doshort);
	}
}

function formathint_html($eword,$ansformats,$reqdecimals,$calledfrom, $islist=false,$doshort=false) {
	$tip = '';
	$shorttip = '';
	if (in_array('set',$ansformats) || in_array('exactset',$ansformats)) {
		$listtype = "set";
...
function formathint_latex($eword,$ansformats,$reqdecimals,$calledfrom, $islist=false,$doshort=false) {
	$tip = '';
	$shorttip = '';
	if (in_array('set',$ansformats) || in_array('exactset',$ansformats)) {
		$listtype = "set";
	...
	if (in_array('fraction',$ansformats)) {
		...
		if (in_array("allowunreduced",$ansformats)) {
			$tip .= sprintf(_('Enter %s as a mixed number or as an integer.  Example: 2 1/2 = \\[2\\frac{1}{2}\\]'), $eword);
		} else {
			$tip .= sprintf(_('Enter %s as a reduced mixed number or as an integer.  Example: 2 1/2 = \\[2\\frac{1}{2}\\]'), $eword);
		}
		$shorttip = $islist?sprintf(_('Enter a %s of mixed numbers or integers'), $listtype):_('Enter a mixed number or integer');
	} else if (in_array('mixednumberorimproper',$ansformats)) {
		if (in_array("allowunreduced",$ansformats)) {
			$tip .= sprintf(_('Enter %s as a mixed number, proper or improper fraction, or as an integer.  Example: 2 1/2 = \\[2\\frac{1}{2}\\]'), $eword);
			$shorttip = $islist?sprintf(_('Enter a %s of mixed numbers, fractions, or integers'), $listtype):_('Enter a mixed number, proper or improper fraction, or integer');
		} else {
			$tip .= sprintf(_('Enter %s as a reduced mixed number, reduced proper or improper fraction, or as an integer.  Example: 2 1/2 = \\[2\\frac{1}{2}\\]'), $eword);
			$shorttip = $islist?sprintf(_('Enter a %s of mixed numbers, fractions, or integers'), $listtype):_('Enter a reduced mixed number, proper or improper fraction, or integer');
		}
	} else if (in_array('fracordec',$ansformats)) {
		if (in_array("allowmixed",$ansformats)) {
	...
	if (in_array('nodecimal',$ansformats)) {
		...
		if ($reqdecimals == 0) {
			$tip .= "\n\n" . sprintf(_('Enter %s accurate to the nearest integer.'), $eword);
		} else {
			$tip .= "\n\n" . sprintf(_('Enter %s accurate to %d decimal places.'), $eword, $reqdecimals);
		}
	}
	if (in_array('notrig',$ansformats)) {
		$tip .= "\n\n" . _('Trig functions (sin,cos,etc.) are not allowed');
	}
	if ($doshort) {
		return array($tip,$shorttip);
	} else {
