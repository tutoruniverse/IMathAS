    ...
    if ($_SESSION['graphdisp'] == 0) {
        ...
        foreach ($funcs as $f) {
            ...
            foreach ($parts as &$part) {
                if (substr($part, 0, 1) === '[') {
                    $part = '[' . makexxpretty(substr($part, 1));
                } else {
            ...
            unset($part);
            $function_names[] = trim(implode(',', $parts));
        }
        $function_list_str = json_encode($function_names);

        $gs_args = func_get_args();
        $gs = call_user_func_array('buildGraphSettingsJson', $gs_args);
        ...
        // NOTE: $gs['plot_data'] (base64-encoded view_window/axes/grid/canvas config) is
        // available here for future use but not yet injected into the embed tag.
        return "<embed type='image/svg+xml' align='middle' width='$plotwidth' height='$plotheight' function_list='$function_list_str' plot-func='{$gs['plot_func']}' script='$commands' />\n";
    }
}

function buildGraphSettingsJson($funcs) { //optional arguments:  $xmin,$xmax,$ymin,$ymax,labels,grid,width,height
    if (!is_array($funcs)) {
        settype($funcs, "array");
    }
    $settings = array(-5, 5, -5, 5, 1, 1, 200, 200);
    ...
    foreach ($funcs as $function) {
        if (is_array($function) || $function == '') { continue; }
        if (substr($function, 0, 4) == 'alt:') { continue; }
        $function = str_replace('\\,', '&x44;', $function);
        $function = listtoarray($function);
    ...
    $gs_functions = $graph_settings['functions'];
    unset($graph_settings['functions']);
    return [
        'plot_data' => base64_encode(json_encode($graph_settings)),
        'plot_func' => base64_encode(json_encode($gs_functions))
    ];
}
