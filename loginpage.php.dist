...
//any extra CSS, javascript, etc needed for login page
	...
	if (!empty($CFG['allow_passkeys'])) {
		$placeinhead .= "<script type=\"text/javascript\" src=\"$imasroot/javascript/passkey-login.js\" ></script>";
	}
	$nologo = true;
	require("header.php");
...
if (!empty($CFG['allow_passkeys'])) {
?>
<div id="passkeyentry" style="display:none">
<div class="separator"><span>or</span></div>
<p id="passkeyerror" class="noticetext" style="display:none"></p>
<div class=textright><button type=button onclick="loginWithPasskey()">Login with Passkey</button></div>
<input type="hidden" id="passkeyCredentialId" name="passkeyCredentialId" value="">
<input type="hidden" id="passkeyClientDataJSON" name="passkeyClientDataJSON" value="">
<input type="hidden" id="passkeySignature" name="passkeySignature" value="">
<input type="hidden" id="passkeyAuthenticatorData" name="passkeyAuthenticatorData" value="">
</div>
<?php
}
?>
